(function (global) {
  ...
  function drawJourneyService(ctx, data, c) {
    ...
    drawDiamond(ctx, 844, 684, 8, c.gold);
    drawDiamond(ctx, 1083, 684, 8, c.gold);
    ctx.fillStyle = c.ink;
    ctx.font = font(22, 900, 'Arial, Helvetica, sans-serif');
    ctx.fillText('JOURNEY SERVICE', 875, 692);

    ctx.fillStyle = c.paper;
    ctx.strokeStyle = c.line;
    roundedRect(ctx, 530, 710, 1180, 104, 18);
    ctx.fill();
    ctx.stroke();

    ctx.strokeStyle = c.lineSoft;
    ctx.beginPath();
    ctx.moveTo(875, 730); ctx.lineTo(875, 794);
    ctx.moveTo(1160, 730); ctx.lineTo(1160, 794);
    ctx.stroke();

    drawCupIcon(ctx, 570, 761, 0.70, c.navyMid);
    drawSugarIcon(ctx, 920, 761, 0.72, c.navyMid);
    drawFoodIcon(ctx, 1207, 761, 0.72, c.navyMid);

    ctx.fillStyle = c.gold;
    ctx.font = font(13, 800, 'Arial, Helvetica, sans-serif');
    ctx.fillText('DRINK', 616, 748);
    ctx.fillText('SWEETNESS', 968, 748);
    ctx.fillText('FOOD', 1256, 748);

    ctx.fillStyle = c.ink;
    drawWrappedText(ctx, data.drink, 616, 781, 235, 2, 16, 11, 800, 'Tahoma, Arial, sans-serif', 1.15);
    drawWrappedText(ctx, data.sweetness, 968, 781, 160, 2, 17, 12, 800, 'Tahoma, Arial, sans-serif', 1.15);
    drawWrappedText(ctx, data.food, 1256, 781, 425, 2, 16, 11, 800, 'Tahoma, Arial, sans-serif', 1.15);
  }
